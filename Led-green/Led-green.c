...
int main(int argc, char *argv[]) {
	...
	val=in32(gpio_base+LED_GPFSEL_GREEN);
	val&=~(7<<LED_GPFBIT_GREEN);
	val|=1 << LED_GPFBIT_GREEN;
	out32(gpio_base+LED_GPFSEL_GREEN,val);

	while(1)
	{
		delay(1000);
		out32(gpio_base+LED_GPCLR,1<<LED_GPIO_BIT_GREEN);
		delay(1000);
		out32(gpio_base+LED_GPSET,1 << LED_GPIO_BIT_GREEN);
	}
